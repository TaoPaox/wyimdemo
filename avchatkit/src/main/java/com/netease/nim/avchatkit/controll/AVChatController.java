...
public class AVChatController {
    ...
    public AVChatController(Context context, AVChatData avChatData) {
        Log.i("AV_CHAT","----AVChatController    avChatData==null:"+(avChatData==null)+"    context:"+context.getClass().toString());
    ...
    public void doCalling(String account, final AVChatType avChatType, final AVChatControllerCallback<AVChatData> callback) {
        ...
        if (mVideoCapturer == null) {
            mVideoCapturer = AVChatVideoCapturerFactory.createCameraCapturer(true);   // true  --> default front camera
            AVChatManager.getInstance().setupVideoCapturer((AVChatVideoCapturer) mVideoCapturer);
        }
        ...
        // 默认forceKeepCalling为true，开发者如果不需要离线持续呼叫功能可以将forceKeepCalling设为false
         notifyOption.forceKeepCalling = true;
        ...
        AVChatManager.getInstance().call2(account, avChatType, notifyOption, new AVChatCallback<AVChatData>() {
            ...
            public void onFailed(int code) {
                ...
                Log.i("AV_CHAT","  --------P2P  onFailed---- code:"+code);
            ...
            public void onException(Throwable exception) {
                Log.i("AV_CHAT","  --------P2P  onException---- exception:"+Log.getStackTraceString(exception));
    ...
    public void toggleRecord(int type, final String receiverId, final RecordCallback callback) {
        if (isRecording) {
            ...
            //探测对话框
            final MultiSelectDialog selectDialog = new MultiSelectDialog(getContext());
            selectDialog.setTitle("选择录制内容");
            selectDialog.setMessage("录制的内容会被单独保存");
            selectDialog.setMessageTextColor(getContext().getResources().getColor(R.color.color_grey_999999));
            ...
            selectDialog.addNegativeButton(getContext().getString(R.string.cancel), MultiSelectDialog.NO_TEXT_COLOR,
    ...
    public void showQuitToast(int code) {
        switch (code) {
            ...
            case AVChatExitCode.CONFIG_ERROR: // 服务器返回数据错误
                Toast.makeText(getContext(), R.string.avchat_net_error_then_quit, Toast.LENGTH_SHORT).show();
                break;
            case AVChatExitCode.REJECT:
                Toast.makeText(getContext(), R.string.avchat_call_reject, Toast.LENGTH_SHORT).show();
                break;
            case AVChatExitCode.PEER_HANGUP:
            case AVChatExitCode.HANGUP:
                if (isCallEstablish.get()) {
                    Toast.makeText(getContext(), R.string.avchat_call_finish, Toast.LENGTH_SHORT).show();
                }
                break;
            case AVChatExitCode.PEER_BUSY:
                Toast.makeText(getContext(), R.string.avchat_peer_busy, Toast.LENGTH_SHORT).show();
                break;
            case AVChatExitCode.PROTOCOL_INCOMPATIBLE_PEER_LOWER:
                Toast.makeText(getContext(), R.string.avchat_peer_protocol_low_version, Toast.LENGTH_SHORT).show();
                break;
            case AVChatExitCode.PROTOCOL_INCOMPATIBLE_SELF_LOWER:
                Toast.makeText(getContext(), R.string.avchat_local_protocol_low_version, Toast.LENGTH_SHORT).show();
                break;
            case AVChatExitCode.INVALIDE_CHANNELID:
                Toast.makeText(getContext(), R.string.avchat_invalid_channel_id, Toast.LENGTH_SHORT).show();
                break;
            case AVChatExitCode.LOCAL_CALL_BUSY:
                Toast.makeText(getContext(), R.string.avchat_local_call_busy, Toast.LENGTH_SHORT).show();
                break;
            default:
                break;
        }
